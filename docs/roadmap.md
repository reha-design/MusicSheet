...
| 상태 | 작업 | 완료 기준 | 관련 사양 |
| :--- | :--- | :--- | :--- |
| Planned | LocalStorage 어댑터 구현 | `ArtifactStorage`의 `put`, `open_read`, `exists`, `materialize`를 구현하고, `put`이 `ArtifactRef`를 반환하며 `outputs/{job_id}/` 구조에 저장합니다. | [스토리지 추상화](architecture/storage.md), [아티팩트 규격](domain/artifacts.md) |
